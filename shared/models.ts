...
export function createIssue(data: Issue): Issue {
  return issueSchema.parse(data);
}

export function createIssueEvaluation(
  data: Omit<IssueEvaluation, "createdAt" | "updatedAt">,
): IssueEvaluation {
  const now = new Date().toISOString();
  return issueEvaluationSchema.parse({
    ...data,
    createdAt: now,
    updatedAt: now,
  });
...
export function applyIssueEvaluationUpdate(
  existing: IssueEvaluation,
  updates: Partial<IssueEvaluation>,
): IssueEvaluation {
  return issueEvaluationSchema.parse({
    ...existing,
    ...updates,
