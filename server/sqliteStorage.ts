...
type IssueEvaluationRow = {
  target_id: string;
  repo: string;
  issue_number: number;
  status: IssueEvaluation["status"];
  confidence: number;
  summary: string;
  safety_flags_json: string;
  recommended_labels_json: string;
...
export class SqliteStorage implements IStorage {
  ...
  private throwPersistentLockError(error: SqliteError): never {
    const wrapped = new Error(
      `SQLite database remained locked after recovery attempts at ${this.stateDbPath}. Stop the competing patchdeck process or use a different PATCHDECK_HOME.`,
      { cause: error },
    ) as SqliteError;
  ...
  private bootstrap(): void {
    this.exec(`
      ...
      CREATE TABLE IF NOT EXISTS issue_evaluations (
        ...
        recommended_labels_json TEXT NOT NULL,
        marker_comment_id INTEGER,
        created_at TEXT NOT NULL,
        updated_at TEXT NOT NULL
      );
    ...
    this.migrateLegacyLogForeignKey();
  ...
  private migrateLegacyLogForeignKey(): void {
    const foreignKeys = this.all<{ table: string; from: string; to: string }>(`PRAGMA foreign_key_list(logs)`);
    const hasLegacyPrForeignKey = foreignKeys.some((row) => row.table === "prs" && row.from === "pr_id" && row.to === "id");
    if (!hasLegacyPrForeignKey) {
      return;
    }

    this.withWriteTransaction(() => {
      this.exec("ALTER TABLE logs RENAME TO logs_legacy");
      this.exec(`
        CREATE TABLE logs (
          id TEXT PRIMARY KEY,
          pr_id TEXT NOT NULL,
      ...
      this.exec(`
        ...
        FROM logs_legacy
      `);
      this.exec("DROP TABLE logs_legacy");
    });
  }
  ...
  private parseIssueEvaluationRow(row: IssueEvaluationRow): IssueEvaluation {
    return {
      targetId: row.target_id,
      repo: row.repo,
      ...
      summary: row.summary,
      safetyFlags: parseStringArrayJson(row.safety_flags_json),
      recommendedLabels: parseStringArrayJson(row.recommended_labels_json),
  ...
  async getIssueEvaluation(targetId: string): Promise<IssueEvaluation | undefined> {
    const row = this.get<IssueEvaluationRow>(`
      SELECT target_id, repo, issue_number, status, confidence, summary,
             safety_flags_json, recommended_labels_json, marker_comment_id,
             created_at, updated_at
      FROM issue_evaluations
      WHERE target_id = ?
    `, targetId);

    return row ? this.parseIssueEvaluationRow(row) : undefined;
  }

  async upsertIssueEvaluation(
    evaluation: Omit<IssueEvaluation, "createdAt" | "updatedAt">,
  ): Promise<IssueEvaluation> {
    const existing = await this.getIssueEvaluation(evaluation.targetId);
    const next = existing
      ...
      this.run(
        `
          INSERT INTO issue_evaluations (
            target_id, repo, issue_number, status, confidence, summary,
            safety_flags_json, recommended_labels_json, marker_comment_id,
            created_at, updated_at
          )
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          ON CONFLICT(target_id) DO UPDATE SET
            ...
            recommended_labels_json = excluded.recommended_labels_json,
            marker_comment_id = excluded.marker_comment_id,
        ...
        next.confidence,
        next.summary,
        JSON.stringify(next.safetyFlags),
        JSON.stringify(next.recommendedLabels),
        next.markerCommentId,
        next.createdAt,
        next.updatedAt,
      );
    });
