...
test("listOpenIssuesForRepo filters pull requests and normalizes issue metadata", async () => {
  const octokit = {
    paginate: async () => [
      {
        number: 17,
...
test("fetchIssueSummary normalizes direct issue metadata", async () => {
  ...
  const issue = await fetchIssueSummary(octokit as never, { owner: "owner", repo: "repo", number: 19 });

  assert.equal(issue.number, 19);
  assert.equal(issue.title, "Fix the footer");
...
test("fetchFeedbackItemsForPR marks patchdeck status and audit comments non-actionable at ingest", async () => {
      ...
      if (method === listIssueComments) {
        return [
          ...
          },
          {
            ...
            html_url: "https://github.com/jeremymcs/patchdeck/pull/1#issuecomment-202",
            user: { login: "octocat", type: "User" },
          },
          {
            id: 204,
          ...
          },
          {
            ...
            html_url: "https://github.com/jeremymcs/patchdeck/pull/1#issuecomment-203",
...
test("fetchFeedbackItemsForPR paginates review thread comments beyond the first page", async () => {
        ...
        return [
          {
            ...
            line: 331,
            created_at: "2026-03-15T11:00:00Z",
            html_url: "https://github.com/jeremymcs/patchdeck/pull/1#discussion_r101",
            user: {
  ...
  const items = await fetchFeedbackItemsForPR(
    octokit as never,
    { owner: "jeremymcs", repo: "codefactory", number: 1 },
    config,
  );
