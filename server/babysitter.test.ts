...
test("babysitPR stages, commits, and pushes agent file edits from the app runtime", async () => {
  const storage = new MemStorage();
  await storage.updateConfig({ autoUpdateDocs: false });
  const worktreeRoot = await mkdtemp(path.join(os.tmpdir(), "patchdeck-test-"));
  process.env.CODEFACTORY_HOME = worktreeRoot;
  const existingItem = makeFeedbackItem();
  const pr = await storage.addPR({
    number: 42,
    ...
    },
    {
      ...
      runCommand: async (command, args) => {
        ...
        if (args[0] === "commit") {
          ...
          return { code: 0, stdout: "[feature/verbose appcommit1] Apply PatchDeck fixes\n", stderr: "" };
        }
        if (args[0] === "rev-parse" && args[1] === "HEAD") {
          return { code: 0, stdout: `${localHeadSha}\n`, stderr: "" };
        }
  ...
  assert.ok(gitCommands.includes("commit --no-verify -m Apply PatchDeck fixes for PR #42"));
...
test("runQueuedBabysitPR recovers an existing status reply after restart", async () => {
  ...
  const staleStatusReply = makeFeedbackItem({
    ...
    decision: "reject",
    decisionReason: "PatchDeck status comment",
    status: "rejected",
    statusReason: "PatchDeck status comment",
  });
  const pr = await storage.addPR({
    number: 106,
    title: "Verbose PR",
...
test("babysitPR treats footerless status replies as non-actionable", async () => {
  ...
  assert.match(updated?.feedbackItems[0]?.statusReason ?? "", /PatchDeck status comment/i);
});
...
test("isTrustedReviewer matches authors case-insensitively and ignores @ prefix", async () => {
  const { isTrustedReviewer } = await import("./babysitter");

  // Empty list matches nothing.
  assert.equal(isTrustedReviewer("octocat", []), false);

  // Empty author matches nothing.
  assert.equal(isTrustedReviewer("", ["octocat"]), false);

  // Exact match.
  assert.equal(isTrustedReviewer("octocat", ["octocat"]), true);

  // Case-insensitive on either side.
  assert.equal(isTrustedReviewer("OctoCat", ["octocat"]), true);
  assert.equal(isTrustedReviewer("octocat", ["OCTOCAT"]), true);

  // Leading @ is stripped on both author and entries.
  assert.equal(isTrustedReviewer("@octocat", ["octocat"]), true);
  assert.equal(isTrustedReviewer("octocat", ["@octocat"]), true);
  assert.equal(isTrustedReviewer("@octocat", ["@octocat"]), true);
  ...
  assert.equal(isTrustedReviewer("octo", ["octocat"]), false);
  assert.equal(isTrustedReviewer("octocat-bot", ["octocat"]), false);
