...
export function applyLogEnv(log: CliLogOptions | undefined): void {
  ...
  if (log.noLogFile) process.env.PATCHDECK_NO_LOG_FILE = "1";
  if (log.logFile) process.env.PATCHDECK_LOG_FILE = log.logFile;
}
...
export function formatCliHelp(version: string): string {
  ...
  Usage:
    ...
    patchdeck --help       Show this help message
    patchdeck --version    Print the version
  ...
  Environment variables:
    ...
    PATCHDECK_HOME         Override config/state directory (~/.patchdeck)
