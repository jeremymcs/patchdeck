#!/usr/bin/env node
/**
 * mcp.ts — PatchDeck MCP Server
 *
 * Exposes every PatchDeck capability as an MCP (Model Context Protocol)
 * tool so that any MCP-compatible agent (Claude Desktop, OpenClaw, etc.) can
 * drive PatchDeck without needing to speak raw HTTP.
 *
 ...
 *       "patchdeck": {
 *         "command": "npx",
 *         "args": ["tsx", "/path/to/patchdeck/server/mcp.ts"],
 *         "env": { "PATCHDECK_PORT": "5001" }
 *       }
 *     }
 *   }
 *
...
const APP_PORT = process.env.PATCHDECK_PORT ?? process.env.OH_MY_PR_PORT ?? process.env.CODEFACTORY_PORT ?? "5001";
...
const TOOLS: Tool[] = [
  // ── Repositories ──────────────────────────────────────────────────────────
  {
    name: "list_repos",
    description:
      "List all repositories currently being watched by PatchDeck. " +
  ...
  },
  {
    name: "add_repo",
    description:
      "Add a GitHub repository to the PatchDeck watch list. " +
  ...
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: {
        id: { type: "string", description: "Internal PatchDeck PR ID." },
      },
      required: ["id"],
    },
  },
  ...
  {
    name: "remove_pr",
    description: "Remove a PR from PatchDeck's tracking list.",
    inputSchema: {
      type: "object",
      properties: {
        id: { type: "string", description: "Internal PatchDeck PR ID." },
  ...
  // ── Feedback items ─────────────────────────────────────────────────────────
  {
    ...
    inputSchema: {
      type: "object",
      properties: {
        pr_id: { type: "string", description: "Internal PatchDeck PR ID." },
  ...
  // ── Config ─────────────────────────────────────────────────────────────────
  {
    name: "get_config",
    description:
      "Read the current PatchDeck configuration. GitHub tokens are redacted.",
    inputSchema: { type: "object", properties: {}, required: [] },
  },
  {
    name: "update_config",
    description:
      "Partially update PatchDeck configuration. All fields are optional; only provided " +
  ...
  // ── Runtime ────────────────────────────────────────────────────────────────
  {
    name: "get_runtime",
    description:
      "Get PatchDeck runtime state: drain mode status, active run count, and timestamps.",
    inputSchema: { type: "object", properties: {}, required: [] },
  },
  {
    name: "set_drain_mode",
...
async function main() {
  ...
  process.stderr.write(
    `[patchdeck-mcp] MCP server started — targeting ${APP_BASE_URL}\n`,
  );
}

main().catch((err) => {
  process.stderr.write(`[patchdeck-mcp] Fatal: ${err}\n`);
  process.exit(1);
});
