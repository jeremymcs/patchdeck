...
test("applyLogEnv writes the expected environment variables", () => {
  const prev = {
    ...
    PATCHDECK_LOG_FILE: process.env.PATCHDECK_LOG_FILE,
    PATCHDECK_NO_LOG_FILE: process.env.PATCHDECK_NO_LOG_FILE,
  };
  delete process.env.LOG_LEVEL;
  delete process.env.PATCHDECK_LOG_FILE;
  delete process.env.PATCHDECK_NO_LOG_FILE;

  try {
    applyLogEnv({ level: "warn", logFile: "/tmp/foo.log" });
    assert.equal(process.env.LOG_LEVEL, "warn");
    assert.equal(process.env.PATCHDECK_LOG_FILE, "/tmp/foo.log");
    assert.equal(process.env.PATCHDECK_NO_LOG_FILE, undefined);

    applyLogEnv({ noLogFile: true });
    assert.equal(process.env.PATCHDECK_NO_LOG_FILE, "1");

    applyLogEnv(undefined);
  } finally {
    if (prev.LOG_LEVEL === undefined) delete process.env.LOG_LEVEL; else process.env.LOG_LEVEL = prev.LOG_LEVEL;
    if (prev.PATCHDECK_LOG_FILE === undefined) delete process.env.PATCHDECK_LOG_FILE; else process.env.PATCHDECK_LOG_FILE = prev.PATCHDECK_LOG_FILE;
    if (prev.PATCHDECK_NO_LOG_FILE === undefined) delete process.env.PATCHDECK_NO_LOG_FILE; else process.env.PATCHDECK_NO_LOG_FILE = prev.PATCHDECK_NO_LOG_FILE;
  }
});
