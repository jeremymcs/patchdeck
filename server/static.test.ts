...
test("serveStatic fallback serves index.html from hidden install paths", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), ".patchdeck-static-"));
  const distPath = path.join(root, "public");
  await mkdir(distPath, { recursive: true });
  await writeFile(path.join(distPath, "index.html"), "<!doctype html><title>patchdeck</title>", "utf8");
  ...
  try {
    ...
    try {
      ...
      assert.match(await response.text(), /patchdeck/);
    } finally {
