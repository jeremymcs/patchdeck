...
test("SqliteStorage migrates legacy log foreign keys and accepts issue-target logs", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "codefactory-storage-"));
  const db = createRawDatabase(root);
  ...
  try {
    ...
    db.prepare("INSERT INTO prs (id, repo, number) VALUES (?, ?, ?)").run("legacy-pr", "acme/widgets", 17);
    db.prepare(`
    ...
    const storage = new SqliteStorage(root);
    try {
      await storage.addLog("acme/widgets#17", "info", "Queued manual issue work", {
        metadata: { issueNumber: 17 },
      });

      const legacyLogs = await storage.getLogs("legacy-pr");
      const issueLogs = await storage.getLogs("acme/widgets#17");

      assert.equal(legacyLogs.length, 1);
      assert.equal(legacyLogs[0]?.message, "Legacy PR log");
      assert.equal(issueLogs.length, 1);
      assert.equal(issueLogs[0]?.message, "Queued manual issue work");
    } finally {
      storage.close();
    }
  } finally {
...
test("SqliteStorage upsertAgentRun preserves the original createdAt", async () => {
  ...
  const pr = await storage.addPR({
    ...
    author: "claude",
    url: "https://github.com/jeremymcs/patchdeck/pull/59",
    status: "watching",
    feedbackItems: [],
    accepted: 0,
    rejected: 0,
