# Native App Framework Evaluation for patchdeck

## Context

patchdeck is currently a web application (React frontend + Express backend) that runs locally and opens in the user's browser. We want to convert it into a native desktop app with the following critical requirement:

- **Single-instance enforcement**: Only one instance of patchdeck may run on the OS at a time.
...
| **Child process spawning** | Supported via `tauri-plugin-shell` (sidecar and command execution). Critical for patchdeck's `codex`/`claude` CLI agent spawning. |
...
app := wails.CreateApp(&wails.AppConfig{
    SingleInstanceLock: &options.SingleInstanceLock{
        UniqueId: "com.fluxlabs.patchdeck",
...
**Tauri is the clear winner for patchdeck** for the following reasons:
...
### 3. Practical migration path for patchdeck
...
### 4. Production-ready features patchdeck needs
...
- **System tray**: Run patchdeck in background while babysitting PRs.
- **Notifications**: Alert when PR feedback needs attention.
- **Auto-update**: Push updates to users seamlessly.

### 5. Small footprint
~3-10 MB bundle (vs. Electron's 150+ MB) means fast downloads and low disk usage. ~30-40 MB memory fits patchdeck's "runs in the background" use case.
