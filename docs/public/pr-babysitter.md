...
The PR Babysitter is patchdeck's core feature — an autonomous system that continuously monitors your GitHub pull requests, takes action on review feedback, and can run bounded CI-healing and post-merge deployment-healing loops without manual intervention.

## How It Works

### 1. Repository Watching

When you add a repository to patchdeck, the babysitter begins polling for open pull requests that match that repo's watch scope:
...
Every review comment is captured and stored locally. patchdeck understands GitHub's threaded review model:
...
Not all feedback is equal. patchdeck classifies each piece of feedback into the triage decisions used by the app:
...
For actionable feedback, patchdeck:
...
When a PR branch falls behind the base branch, patchdeck can automatically:
