...
> and API access is disabled unless `PATCHDECK_WEB_USERNAME` and
> `PATCHDECK_WEB_PASSWORD` are set, and then requires a signed dashboard session.
...
`PATCHDECK_SESSION_SECRET` is optional; when omitted, patchdeck creates an
...
Register a GitHub PR by URL. patchdeck fetches the PR summary from GitHub,
...
patchdeck GitHub Actions workflow is installed).
